<template>
    <div class="enterprise-apply">
        <div class="data-container">
            <h4 class="title">注册地工商、税务、建设（质监）、人社、环保等行政主管部门记录的信用情况</h4>
            <div class="table-container">
                ...
                <thead>
                    ...
                    <tbody>
                        ...
                        <tr v-for="item in creditsituationInfo" :key="item.id">
                            <td>{{item.id}}</td>
                            <td>{{item.mainFacts}}</td>
                            <td>{{item.type}}</td>
                            <td>{{item.deparName}}</td>
                            <td>{{item.deparCategory}}</td>
                            <td>{{item.date}}</td>
        ...
        <el-dialog title="信用记录" :visible.sync="addFormVisible">
            <form class="form">
                ...
                <div class="input-container">
                    <label for="deparCategory">处罚部门类型:</label>
                    <input type="text" name="deparCategory" v-model="form.deparCategory">
                </div>
...
export default {
    ...
    created() {
        this.getCreditsituationInfo()
    },
    methods: {
        // 获取信息
         getCreditsituationInfo() {
            let enterid = localStorage.getItem('id')
            this.$http.get(`creditsituation/find.action?user_id=${enterid}`).then(
                res => {
                    if( res.data.responseDesc == '查询成功' ) {
                        this.creditsituationInfo = res.data.responseCode
                    }else {
                    }
                }
            ).then( () => {
                // 判断是否有数据
                if( this.creditsituationInfo.length ) {
        ...
        handleAdd() {
            ...
            // 提交新增
            this.$http({
                url: 'creditsituation/insert.action',
                method: 'post',
                data: formdata
            }).then( res => {
                if( res.data.responseCode == 1 ) {
                    ...
                    this.getCreditsituationInfo()  // 重新获取数据
                }
            } ).catch( err => {
                this.hasNoData = true
                this.$message( {
        ...
        deleteItem(id) {
            this.$http({
                url: `creditsituation/delete.action?id=${id}`,
                method: 'get'
            }).then( res => {
        ...
        handleUpdate() {
            let formData = JSON.stringify(this.form)
            this.$http({
                url: 'creditsituation/update.action',
                method: 'post',
                data: formData
            }).then( res => {
                if( res.data.responseCode == 1 ) {
