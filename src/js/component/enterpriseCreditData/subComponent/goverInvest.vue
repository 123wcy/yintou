<template>
    <div class="enterprise-apply">
        <div class="data-container">
            <h4 class="title">政府投资、国有投资项目中标履约情况</h4>
            <div class="table-container">
                ...
                <thead>
                    ...
                    <tbody>
                        ...
                        <tr v-for="item in goverInvestInfo" :key="item.id">
                            <td>{{item.id}}</td>
...
export default {
    ...
    created() {
        this.getGoverInvestInfo()
    },
    methods: {
        // 获取信息
         getGoverInvestInfo() {
            let enterid = localStorage.getItem('id')
            this.$http.get(`goverinvest/find.action?user_id=${enterid}`).then(
                res => {
                    if( res.data.responseDesc == '查询成功' ) {
                        console.log(res.data)
                        this.goverInvestInfo = res.data.responseCode
                    }else {
                    }
                }
            ).then( () => {
                // 判断是否有数据
                if( this.goverInvestInfo.length ) {
        ...
        handleAdd() {
            ...
            this.$http({
                ...
                if( res.data.responseCode == 1 ) {
                    ...
                    this.getGoverInvestInfo()  // 重新获取数据
                }
            } ).catch( err => {
                this.hasNoData = true
                this.$message( {
        ...
        deleteItem(id) {
            this.$http({
                url: `goverinvest/delete.action?id=${id}`,
                method: 'get'
            }).then( res => {
